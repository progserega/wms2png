...
do
  ...
  then
    ...
    echo "conf_file=" >> "${log}"
    cat $conf_file >> "${log}"
    sendEmail -s $email_server -f $email_from -t $email_result_to -u "error process map create by your request" -m "Карта для вас не сгенерирована. Возможно проблема во входных данных от вас или дело во внутренних проблемах в системе. Обратитесь к системному администратору" 
    rm "$conf_file"
    exit 1
  fi
  ...
  sendEmail -s $email_server -f $email_from -t $email_result_to -u "your map is success!" -m "Карта для вас сгенерирована. Вы можете скачать архив со сгенерированными для вас слоями по ссылке:  $export_url/$tar_name"
